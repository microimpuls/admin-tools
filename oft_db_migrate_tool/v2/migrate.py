...
- позволяет также перенести таблицу collocations (понятие Дата-центр в Smarty)
- переносит MAC-адреса привязанных устройств
- включает авторизацию по UID устройства всем аккаунтам

Это позволяет мигрировать базу аккаунтов без восстановления паролей.

(c) Konstantin Shpinev, 2016
"""
...
def insert_customer(customer):
    ...
    smarty_cur.execute("SELECT * FROM tvmiddleware_customer WHERE firstname = '%s' AND lastname = '%s' AND lastname NOT LIKE 'Auto %s'" %
        (customer['firstname'], customer['lastname'], '%'))
    if smarty_cur.rowcount > 0:
        return None
    smarty_cur.execute("SELECT * FROM tvmiddleware_customer WHERE email = '%s'" %
        (customer['email']))
    if smarty_cur.rowcount > 0:
        customer['email'] = ""
    postal_country = "NULL"
...
#        billing_country = config.COUNTRY_ID
    ...
    phone_number_1, fax_phone_number, email, company_name, comment, client_id, dealer_id, gender, balance, currency_id, \
    default_payment_type) \
    VALUES(\"%s\", \"%s\", \"%s\", \"%s\", %s, \"%s\", %s, %s, \"%s\", %s, \"%s\", %s, %s, \"%s\", \"%s\", %s, \"%s\", \"%s\", %s, %s, %s, 0.0, %d, %d)" % \
    (customer['firstname'], customer['lastname'], customer['birthdate'], customer['postal_address_street'],
     ...
     config.CURRENCY_ID, config.DEFAULT_PAYMENT_TYPE)
    try:
        sql = sql.replace('\\"', '"')
...
def insert_account(account):
    ...
    allow_login_by_abonement, views_count, data_center_id, allow_login_by_device_uid, use_timeshift) \
    VALUES('%s', '%s', %d, %s, DATE_ADD(%s, INTERVAL 1 MONTH), '%s', %s, '%s', %s, '%s', %s, %s, 0, 0, 0, %s, 1, %d)" % \
    (account['login'], encode_password(account['password']), 1 if account['active'] else 0, clean_date(account['activation_date']),
     ...
     clean_date(account['created_at'], True), account['ip'], int(account['customer_id']), config.CLIENT_ID, data_center, use_timeshift)
    try:
        smarty_cur.execute(sql)
        smarty_db.commit()
        return smarty_cur.lastrowid
...
# Return new data_center_id related to oft collocation_id
def map_data_center_id(id):
    try:
        return config.DATACENTER_ID_MAP[str(id)]
    except:
        return None
...
for id, customer in customers.iteritems():
    oft_cur.execute("SELECT \
    u.id, u.login, u.pass, u.ip_addr, u.channel_pass, s.mac, s.stb_type_id, u.collocation_id, u.channel_offset \
    FROM iptv_Users u \
    ...
    for row in oft_cur.fetchall():
        ...
        data_center_id = map_data_center_id(row[7])
        use_timeshift = 1 if row[8] != '0' else 0
...
try:
    ...
    oft_cur.execute("SELECT \
        d.id, d.register, d.tax_number, d.value_added_tax_number, d.city, d.street, d.register as 'name' \
        FROM iptv_Dealers d")
for row in oft_cur.fetchall():
    id = row[0]
    register = row[1]
